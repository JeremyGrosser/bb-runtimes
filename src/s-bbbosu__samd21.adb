...
with Interfaces.SAMD.SYSCTRL; use Interfaces.SAMD.SYSCTRL;
with Interfaces.SAMD.MTB;     use Interfaces.SAMD.MTB;
...
package body System.BB.Board_Support is
   ...
   Micro_Trace_Buffer : array (0 .. 256) of Unsigned_32;
   pragma Volatile (Micro_Trace_Buffer);
   ...
   --  Interrupt_Priority value, and 4 is the lowest.

   function To_PRI (P : Integer) return PRI is
     (if P not in Interrupt_Priority then 0
      else PRI (Interrupt_Priority'Last - P + 1));
   ...
   function To_Priority (P : PRI) return Interrupt_Priority is
     (if P = 0 then Interrupt_Priority'Last
      else (Interrupt_Priority'Last - Any_Priority'Base (P) + 1));
   ...
   IP : array (0 .. Interrupt_ID'Last) of PRI
       with Volatile, Address => 16#E000_E040#;
   ...
   begin
      ...
      MTB_Periph.POSITION := (
          Reserved_0_1 => 16#0#,
          WRAP         => False,
          POINTER      => Micro_Trace_Buffer'Address);
      MTB_Periph.FLOW := (
          AUTOSTOP     => False,
          AUTOHALT     => False,
          Reserved_2_2 => 16#0#,
          WATERMARK    => Micro_Trace_Buffer'Last'Address);
      MTB_Periph.MASTER := (
          EN             => True,
          Reserved_10_30 => 16#0#,
          HALTREQ        => False,
